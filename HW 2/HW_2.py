...
# per professor, make sure to not use (exp) to define a function when using packages like math, use f(x).proximating Taylor series
def exp(z, terms=20):  
...
def f(t):  # defining the function we want to integrate
    return exp(-t*t)
